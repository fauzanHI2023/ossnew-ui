...
import { NewsCard } from "./DocumentCard";
...
export function NewsGrid({ documentsItems, hasMore, onLoadMore }: NewsGridProps) {
  return (
    <section className="py-12 lg:py-16">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-8">
        {/* Featured Article */}
        {documentsItems.length > 0 && documentsItems[0].large && (
          <div className="mb-12">
            <NewsCard {...documentsItems[0]} />
          </div>
        )}

        {/* Grid Articles */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
          {documentsItems.slice(documentsItems[0]?.large ? 1 : 0).map((item) => (
